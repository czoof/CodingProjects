...
class Landco:
   ...
   def create_own_password(self, event=None):
       ...
       self.email_and_password.config(text="Enter your password. Password must contain a capital letter, number, special character, and 7 characters long")
   ...
   def submit_own_password(self, event=None):
       ...
       if (any(char in own_password for char in '!@#$%^&*()_+-=') and
           ...
           # Invalid password
           self.email_and_password.config(text="Password must contain a capital letter, number, and special character")
